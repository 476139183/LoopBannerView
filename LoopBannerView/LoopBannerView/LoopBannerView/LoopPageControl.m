...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (rate >= 0.0f && rate <= _numberOfPages - 1) {
    ...
    // 颜色选择
    if (currentView.frame.size.width >= (self.currentIndicatorWidth/3) * 2) {
      currentView.backgroundColor = self.currentPageIndicatorColor;
    } else {
      currentView.backgroundColor = self.pageIndicatorColor;
    }
    
    if (oldView.frame.size.width > (self.currentIndicatorWidth/3) * 2) {
      oldView.backgroundColor = self.currentPageIndicatorColor;
    } else {
      oldView.backgroundColor = self.pageIndicatorColor;
    }
